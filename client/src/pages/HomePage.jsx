...
function HomePage() {
  ...
  const handleSwap = () => {
    ...
    if (result?.translatedCode) { setCode(result.translatedCode); setResult(null); }
  };
  ...
  const handleRun = async () => {
    ...
    if (activeAction === "translate" && !targetLanguage) return toast.error("Select a target language.");
    ...
    <div className="home">
      <div className="toolbar">
        <div className="action-tabs">
          {ACTIONS.map((a) => (
            <button key={a} className={`action-tab ${activeAction === a ? "active" : ""}`}
        ...
        <div className="panel">
          <div className="panel-header">
            {activeAction === "translate"
              ...
              : <span className={`action-badge ${activeAction}`}>{activeAction}</span>
            }
            {result && (
